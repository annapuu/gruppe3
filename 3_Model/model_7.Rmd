...
```{r}
# make predictions with model 9 and compare with actual values of validation data
# model 9, our baseline model
...
predicted_values <- predict(mod9, newdata = validation_data2)
...
The Mean Absolute Percentage Error is 33.8937 which is still quite high and indicates a reasonable forecasting.


```{r}
# make predictions with model 9 for test data
predicted_test_values <- predict(mod9, newdata = test_data)
...
write.csv(submission_filled, "gruppe3_submission_m9.csv", row.names = FALSE) # for nes model 9
